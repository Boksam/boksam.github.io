...
최근 [코드플레이스](https://code.pusan.ac.kr) 프로젝트를 Kubernetes 환경으로 마이그레이션하면서 네트워크 인프라의 가용성을 높이는 것에 중점을 두고 있습니다.

[Kube-vip 기반 고가용성 K3s 클러스터 구축]({{ site.baseurl }}{% link _posts/2025-12-13-kube-vip.md %}) 글에서 다룬 것처럼 저는 `Kube-vip`를 사용하여 HA 클러스터를 구축하였는데요,
이번 글에서는 Kube-vip를 로드밸런서 컨트롤러로 구성하고 Traefik이 외부 트래픽을 받을 수 있도록 구성한 과정에 대해 다뤄보려고 합니다.
...
하지만 저는 K3s 설치 단계에서 `--disable servicelb` 옵션을 사용하여 이를 비활성화했습니다. 그 이유는 다음과 같은데요,

1. **엔드포인트의 단일화 (VIP 관리)**: 코드플레이스 프로젝트는 가상 IP(VIP)를 통해 모든 트래픽을 수용하도록 설계되었습니다. 만약 ServiceLB를 켜두면 클러스터 노드들의 실제 물리 IP가 서비스에 할당될 수 있는데, 이는 VIP를 이용해 Fault Tolerance를 확보하려는 구성 의도와 맞지 않습니다.
...
1. **Annotation 방식**
  - 서비스 매니페스트에 특정 IP를 명시하여 Kube-vip가 해당 IP만 할당하게 하는 방법입니다.
  - 특징: 특정 서비스에 고정 IP를 부여하고 싶을 때 유용합니다.
2. **IP Pool 방식**
  - Kube-vip에 사용 가능한 IP 대역을 명시해두고, 서비스가 생성될 때마다 남는 IP를 자동으로 할당하는 방법입니다.
  - 특징: 관리해야 할 서비스가 많고, IP 자동 할당이 필요할 때 적합합니다. (클라우드 환경과 유사)

코드플레이스 프로젝트에서는 모든 외부 트래픽이 사전에 정의된 **가상 IP(VIP)**로 들어와야 했습니다. 따라서 저는 Annotation 방식을 선택했습니다.

### 3-2. Annotation으로 Traefik에 External IP 지정

이제 Traefik에 Annotation을 지정해줄 차례입니다.
K3s에서 기본 설치되는 Traefik의 설정을 변경하려면 HelmChartConfig 리소스를 생성해야 합니다.

`/var/lib/rancher/k3s/server/manifests/traefik-config.yaml`이라는 파일을 생성하고, 아래와 같이 작성합니다.
